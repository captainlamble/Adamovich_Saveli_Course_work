SELECT
    ...
    SUM(CASE WHEN p.payment_status = 'paid' THEN p.amount ELSE 0 END) AS confirmed_revenue
FROM oltp.cruises c
         JOIN oltp.cruise_seats cs ON c.cruise_number = cs.cruise_number
         JOIN oltp.bookings b ON c.cruise_number = b.cruise_number AND cs.cabin_number = b.seat_number
